    ...
    private static void doScan(int account) throws Exception {
        ...
        for (int i = 0; i < dialogs.size(); i++) {
            ...
            if (DialogObject.isChatDialog(did)) {
                ...
                String uname = ChatObject.getPublicUsername(chat);
                if (uname == null) uname = chat.username;
                String about = chat.about != null ? chat.about : "";
                candidates.add(new Item(did, isChannel ? "channel" : "group", title, uname != null ? uname : "", about));
            } else if (DialogObject.isUserDialog(did)) {
                TLRPC.User user = mc.getUser(did);
                if (user == null || !user.bot) continue;
                String name = UserObject.getUserName(user);
                String uname = UserObject.getPublicUsername(user);
                if (uname == null && user.username != null) uname = user.username;
                candidates.add(new Item(did, "bot", name != null ? name : "", uname != null ? uname : "", ""));
            }
        }
